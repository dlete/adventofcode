...
def find_parts_in_input(lol:list):
    ...
    parts_in_the_input = []
    sum_of_parts_in_the_input = 0

    # Iterate lines, find parts in each line, append to the overall list of parts
    for line_index, line_content in enumerate(lol):
        ...
        if len(parts_in_this_line) > 0:
            # THIS WILL OVERRIDE PARTS THAT ALREADY EXISTED!!
            parts_in_the_input.extend(parts_in_this_line)
            # Sum of parts found this far
            sum_of_parts_in_the_input = sum_of_parts_in_the_input + sum(parts_in_this_line)
    
    # Finish
    print("FIND_PARTS_IN_INPUT: Parts found in the input:", parts_in_the_input)
    print("FIND_PARTS_IN_INPUT: Sum of parts found in the input:", sum_of_parts_in_the_input)
...
def find_parts_in_line_00(lol:list, line_index:int):
        ...
        match character_type:
            case 'a_number':
                ...
                if 'a_symbol' in character_types_around_element:
                    #print("ALERTA, I found a symbol!!! around number:", element_content)
                    can_be_part = True
                #print("can_be_part is:", can_be_part)

                # Always start a number or continue building one
                number_in_progress = True
                #print("number_in_progress has value:", number_in_progress)  
            case _:
                #print("Testing element index", element_index, "which is the character", element_content)
                #if number_in_progress == True and can_be_part == True:
                #    # Convert the number_instance, as is at this point, to a integer
                #    number_instance = int(number_instance)
                #    # Add the number_instance to the list of parts in the line
                ...
                if number_in_progress == True:
                    #print("new logic, number_in_progress is", number_in_progress)
                    #print("interim number is:", number_instance)
                    if can_be_part == True:
                        #print("new logic, can_be_part is", can_be_part)
                        # Convert the number_instance, as is at this point, to a integer
                        number_instance = int(number_instance)

                        # TEST, VERIFY IF IT IS IN THE LIST OR NOT
                        if number_instance in parts_in_line:
                            print("number", number_instance, "is already", parts_in_line.count(number_instance), "time(s) in the list")

                            # To overcome, multiply the number by 2 and remove it from the list
                            new_number_instance = number_instance * 2
                            # Verify if new_number is in the list
                            if new_number_instance in parts_in_line:
                                raise Exception("OH!!")
                            else:
                                parts_in_line.append(new_number_instance)
                                print("number", new_number_instance, "is now", parts_in_line.count(number_instance), "time(s) in the list")

                            # remove the number that was alredy in the list
                            parts_in_line.remove(number_instance)
                            print("number", number_instance, "is now", parts_in_line.count(number_instance), "time(s) in the list")

                            #print("new number instance:", new_number_instance)
                            #number_instance = number_instance*2
                            #print("number instance:", number_instance)

                            #parts_in_line.remove(number_instance)
                            # verify it was removed
                            if number_instance in parts_in_line:
                                print(number_instance, "is STILL in the list anymore")
                            if new_number_instance in parts_in_line:
                                print(new_number_instance, "has been added to the list")

                            # number 664 is repeated
                            #raise Exception("The number", number_instance, "already exists in the list", parts_in_line)
                        else:
                            # Add the number_instance to the list of parts in the line
                            parts_in_line.append(number_instance)
                            #print("Have just added number:", number_instance, "to the list of parts in this line")
                        ...
                        number_instance = ''
                        #print("number instance has been reset to:", number_instance)
                    if can_be_part == False:
                        #print("new logic, can_be_part is:", can_be_part)
                        # reset the interim number
                        number_instance = ''
...
def main():
    ...
    #print("MAIN, PARTS IN INPUT:", parts_in_input)
    sum_of_parts_in_input = sum(parts_in_input)
    print("MAIN: sum of parts in the input is:", sum_of_parts_in_input)
...
def sandbox():
    ...
    sample_row_indexes = [25]
    sum_of_parts = 0
    for sample_row_index in sample_row_indexes:
        parts_in_sample_row = find_parts_in_line(lol, sample_row_index)
        print("parts in row with index:", sample_row_index, "are:", parts_in_sample_row)
        sum_of_parts = sum_of_parts + sum(parts_in_sample_row)
    print("sum of the parts is:", sum_of_parts) # sample of part1 is 4361
...
# at 19:43 answer is too low, 538293

# I say correct number is 538293 - 664 = 537629