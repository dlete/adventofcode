...
def prepare_and_process_data(input_list:list):
    ...
    for line_index, line_content in enumerate(input_list):
        ...
        # add as many times as this card inherits from the previous, card2 inherits 1 copy from the prices of card 1
        #if number_of_price_numbers_in_card > 0:
        #    for i in range(number_of_price_numbers_in_card):
        #        staked_repetitions[i] = staked_repetitions[i] + 1
        for i in range(number_of_price_numbers_in_card):
        ...
        # PREVIOUS
        # Reset the first stack level
        staked_repetitions[0] = 0
        #print("staked_repetitions after reseting the first element, and before shifting:", staked_repetitions)
        
        # SHIFT THE STACK
        for i in range(len(staked_repetitions)-1):
            staked_repetitions[i] = staked_repetitions[i+1]
        # set the last element to empty/zero
        staked_repetitions[-1] = 0
        #print("staked_repetitions after shifting:", staked_repetitions)
        ...
        if number_of_price_numbers_in_card > 0:
            for i in range(number_of_price_numbers_in_card):
                staked_repetitions[i] = staked_repetitions[i] + 1
        #print("staked_repetitions after adding card points of card:", card_index, "is:", staked_repetitions)

        # Add the individual card points to the overall score
        total_points = total_points + card_points_part2

    # Finish
    # part1 sample: 13
    # part1 exercise: 24848
    print("total_points_part1:", total_points_part1)
    return total_points_part1
